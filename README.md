...
<div align="center">
  ...
  <p align="center">
    This project is a conformance testing framework designed to assess an API against the Te Whatu Ora API Standards which are officially published at <a href="https://apistandards.digital.health.nz">https://apistandards.digital.health.nz</a>.
    <br />
</div>
...
Check the installed [git version](https://try.github.io/) is >= 2.32.0
...
```bash
# SSH
git clone git@github.com:tewhatuora/api-standards-conformance.git
-- OR --
# HTTPS
git clone https://github.com/tewhatuora/api-standards-conformance.git
```

Check the installed [node version](https://nodejs.org).

> It is recommended that the latest long-term support (LTS) version is used.

You can use node version manager (`nvm`) to easily manage node versions. You can follow the instructions on the [nvm github site](https://github.com/nvm-sh/nvm#installing-and-updating) to install nvm.
Once installed, set your current node version to the latest supported version:
...
To merge custom features with the base implementation, mount your local directory to `/opt/features`
...
To execute specific specific tags - see 
...
This tool is distributed as a Docker container, or you can build the image yourself.
...
Multiple `.emv` files are supported. 
...
The cucumber syntax supports [tagging](https://cucumber.io/docs/cucumber/api/?lang=javascript#tags).

Tags can be used to control `running` or `restricting` scenarios in a test suite.

**Examples:**

> `yarn test --tags @HNZAS_MUST_USE_AUTHORIZATION_HEADER_FOR_GET_REQUEST` - will run the `@HNZAS_MUST_USE_AUTHORIZATION_HEADER_FOR_GET_REQUEST` **scenario** from the feature [**apistandards.HTTPHeaders.Authorization.feature**](./features/FHIR%20Standards/apistandards.HTTPHeaders.Authorization.feature)

---

> `yarn test --tags "@HNZAS_MUST_USE_ACCEPT_HEADER"` - will run **ALL** of the **scenarios** from the [**apistandards.HTTPHeaders.Accept.feature**](./features/FHir%20Standards/apistandards.HTTPHeaders.Accept.feature)

---

> `yarn test --tags "@HNZAS_MUST_USE_ACCEPT_HEADER or @HNZAS_MUST_USE_AUTHORIZATION_HEADER_FOR_GET_REQUEST"` - will run **ALL** of the **scenarios** from the [**apistandards.HTTPHeaders.Accept.feature**](./features/FHir%20Standards/apistandards.HTTPHeaders.Accept.feature) **AND** the `@HNZAS_MUST_USE_AUTHORIZATION_HEADER_FOR_GET_REQUEST` **scenario** in the [**apistandards.HTTPHeaders.Authorization.feature**](./features/FHIR%20Standards/apistandards.HTTPHeaders.Authorization.feature)

---

> `yarn test --tags "not @HNZAS_MUST_USE_AUTHORIZATION_HEADER_FOR_POST_REQUEST"` will run all implemented **scenarios** in all **features** but **NOT** the `@HNZAS_MUST_USE_AUTHORIZATION_HEADER_FOR_POST_REQUEST` **scenario** in the [**apistandards.HTTPHeaders.Authorization.feature**](./features/FHIR%20Standards/apistandards.HTTPHeaders.Authorization.feature)
...
We welcome contributions to this test suite. If you have a new step definition that you think would be useful, please submit a pull request.
