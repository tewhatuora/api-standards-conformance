...
Feature: Validate resources against SDHR profile
  ...
  Scenario: Valid batch request with mixed resources is accepted. This scenario will test that a valid batch request containing a mix of Condition, Observation, AllergyIntolerance resources can be processed successfully.
    Given a batch bundle payload containing "Condition Observation AllergyIntolerance" resources is created for NHI "ZMW6602" at facility "F38006-B" with local ID "null"
    When the API Consumer requests a new client_credentials access token with scope "system/Condition.crus system/Observation.crus system/AllergyIntolerance.crus"
  ...
  Scenario Outline: Request context variations are accepted for Condition search. This scenario focuses on ensuring the Request-Context header permutations are honoured before any resource lookup occurs.
    ...
    Examples:
      ...
      | system context cleanup  | 1b8200d7-3a8c-4fb6-8e5c-cec4540999d5 |                    |                                                                                                     | delete                  | RECORDMGT     | array            | Sample PMS Integration Application |                  |   110150 | string       | G00001-G      |                   | FZZ999-B           |              |
      | clinical comma parsing  | clinician-22                         |                    | {"use":"official","system":"https://standards.digital.health.nz/ns/hpi-person-id","value":"88YYYD"} | json                    | TREAT         | array            | Jean-Luc Picard                    |                  | PROV     |              | G00001-G      |                   | F38006-B           |              |
