...
When('I create payload variations violating each constraint', function () {
  constraintVariations = this.constraints.map((c) => {
    ...
    const result = evaluate(this.payload, c.expr);
    if (result && result.length > 0 && result.every((x) => x === true) && !c.expr.startsWith('contained') && !c.key.startsWith('dom-') && !c.key.startsWith('ele-')) {
